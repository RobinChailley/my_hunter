...
void	reverse(mob_t *list_mob, int size)
{
	...
	mob_texture = sfTexture_createFromFile(
	"images/game/spritesheet.png", NULL);
	reverse_texture = sfTexture_createFromFile(
	"images/game/spritesheet_reverse.png", NULL);
	int i = 0;
	for (i = 0; i < size ; i++) {
		if (list_mob[i].move.x < 0 )
			sfSprite_setTexture(list_mob[i].sprite,
			reverse_texture, sfTrue);
		else
			sfSprite_setTexture(list_mob[i].sprite,
			mob_texture, sfTrue);
	}
